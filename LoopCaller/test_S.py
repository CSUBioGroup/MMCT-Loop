...
def combineCluster(dataS, dataS_2):
    """
    Combining multiple clustering result.
    """
    for key in dataS_2.keys():
        if key not in dataS:
            dataS[key] = {
                "f": dataS_2[key]["f"],
                "records": dataS_2[key]["records"]
            }
        else:
            ds = set()
            for r in dataS[key]["records"]:
                t = [r[1], r[2], r[4], r[5]]
                ds.add(tuple(t))
            for r in dataS_2[key]["records"]:
...
def callpvalue(dataS, minPts, cut, cpu, fout):
    """
    ...
    ds = Parallel(n_jobs=cpu)(delayed(getIntSig)(dataS[key]["f"], dataS[key]["records"], minPts, cut)
    ...
    try:
        ...
        ds = markIntSig(ds)
        ds.to_csv(fout + "_self_ligation.loop", sep="\t", index_label="loopId")
    except:
        log.warning(
...
def startCalPvalue():
    ...
    report = "Command line: test_S.py -f {} -o {} -minPts {} -cpu {} -chr {} -cut {} -tf {}".format(
