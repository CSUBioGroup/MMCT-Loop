...
def startCalPvalue():
    ...
    en_time = time.time()
    log.info("MMCT_Loop estimating inter-ligation loops finished at: " + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(float(en_time)))))
    log.info("MMCT_Loop estimating inter-ligation loops total used %s real cpu time." % (int(en_time - st_time)))
